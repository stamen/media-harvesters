...
function queryInstagramAPI(region, each, done) {
  console.log("[*] Instagram / region %d (%d, %d) @ %dm (last checked @ %s)", region.id, region.lat.toFixed(4), region.lng.toFixed(4), region.radius, region.last_fetched.toISOString());

  var since = (region.last_fetched.getTime() / 1000) | 0;
  ...
  return operation.attempt(function(currentAttempt) {
    return request({
      ...
      if (response.statusCode === 400 && body.meta && body.meta.error_type && body.meta.error_message ) {
        return done(new Error(body.meta.error_type + ": " + body.meta.error_message));
      }

      return done(new Error("Unrecognized error: " + response.statusCode + ", : " + JSON.stringify(body)));
    });
  });
}
  ...
  return _(function(push, next) {
    return pg.connect(DATABASE_URL, function(err, client, done) {
      ...
      var query = client.query.bind(client),
          select = [
              "SELECT id, ST_X(center) AS lng, ST_Y(center) AS lat, radius, COALESCE(last_fetched, NOW() - interval '1 year') AS last_fetched",
  ...
  return pg.connect(DATABASE_URL, function(err, client, done) {
    ...
    var query = client.query.bind(client),
        insert = [
          ...
          "    geom",
          "  FROM (",
          "    SELECT GetIntersectingHexagons(ST_Buffer(ST_Transform(ST_SetSRID(ST_MakePoint($2, $3), 4326), (SELECT ST_SRID(geom) FROM superunits LIMIT 1)), $4), $5) geom",
          "  ) AS hexes",
          // avoid boxiness
